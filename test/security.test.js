...
`;
            ...
            const hasEvalIssue = result.vulnerabilities.some((vuln) => vuln.type === 'no-eval' || vuln.description.toLowerCase().includes('eval'));
...
`;
            ...
            const result = await checkSecurity(filePath, 'javascript', config);
            assert(result.vulnerabilities.length > 0);
            const hasFuncIssue = result.vulnerabilities.some((vuln) => vuln.type === 'no-eval' || vuln.description.toLowerCase().includes('function'));
...
`;
            ...
            const result = await checkSecurity(filePath, 'javascript', config);
            assert(result.vulnerabilities.length > 0);
            const hasXssIssue = result.vulnerabilities.some((vuln) => vuln.type === 'no-xss' || vuln.description.toLowerCase().includes('xss') ||
...
`;
            ...
            const result = await checkSecurity(filePath, 'javascript', config);
            assert(result.vulnerabilities.length > 0);
            const hasSecretIssue = result.vulnerabilities.some((vuln) => vuln.type === 'no-hardcoded-secrets' ||
...
`;
            ...
            const result = await checkSecurity(filePath, 'javascript', config);
            assert(result.vulnerabilities.length > 0);
            const hasSqliIssue = result.vulnerabilities.some((vuln) => vuln.type === 'no-sqli' || vuln.description.toLowerCase().includes('sql injection'));
...
`;
            ...
            const result = await checkSecurity(filePath, 'python', config);
            assert(result.vulnerabilities.length > 0);
            const hasShellIssue = result.vulnerabilities.some((vuln) => vuln.type === 'no-shell' || vuln.description.toLowerCase().includes('shell') ||
