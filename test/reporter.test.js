...
import { generateReport, generateJsonReport, generateHtmlReport, generateMarkdownReport, generateXmlReport, generateConsoleReport, DEFAULT_REPORT_CONFIG, validateReportOutput, generateMultipleReports } from '../dist/reporter/reporter.js';
describe('reporter module', () => {
    ...
    describe('generateJsonReport', () => {
        ...
        it('should include recommendations when configured', () => {
            const result = getMockVerificationResult();
            const config = { ...DEFAULT_REPORT_CONFIG, includeRecommendations: true };
        ...
        it('should not include recommendations when disabled', () => {
            const result = getMockVerificationResult();
            const config = { ...DEFAULT_REPORT_CONFIG, includeRecommendations: false };
            const json = generateJsonReport(result, config);
            const parsed = JSON.parse(json);
        ...
        it('should include timestamp when configured', () => {
            const result = getMockVerificationResult();
            const config = { ...DEFAULT_REPORT_CONFIG, includeTimestamp: true };
            const json = generateJsonReport(result, config);
            const parsed = JSON.parse(json);
        ...
        it('should not include timestamp when disabled', () => {
            const result = getMockVerificationResult();
            const config = { ...DEFAULT_REPORT_CONFIG, includeTimestamp: false };
            const json = generateJsonReport(result, config);
            const parsed = JSON.parse(json);
            assert(!parsed.generatedAt);
        });
        it('should include details when configured', () => {
            const result = getMockVerificationResult();
            const config = { ...DEFAULT_REPORT_CONFIG, includeDetails: true };
            const json = generateJsonReport(result, config);
            const parsed = JSON.parse(json);
        ...
        it('should not include details when disabled', () => {
            const result = getMockVerificationResult();
            const config = { ...DEFAULT_REPORT_CONFIG, includeDetails: false };
            const json = generateJsonReport(result, config);
            const parsed = JSON.parse(json);
            assert(!parsed.details);
        });
        it('should include metrics when configured', () => {
            const result = getMockVerificationResult();
            const config = { ...DEFAULT_REPORT_CONFIG, includeMetrics: true };
            const json = generateJsonReport(result, config);
            const parsed = JSON.parse(json);
    ...
    describe('generateReport', () => {
        ...
        it('should throw error for unsupported format', async () => {
            ...
            await assert.rejects(async () => {
                await generateReport(result, outputPath, { type: 'txt' });
