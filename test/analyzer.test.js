...
`;
            const filePath = createTestFile(testDir, 'test.js', code);
            const result = await analyzeCode(filePath, 'javascript', {});
            const hasStyleIssue = result.codeQuality.issues.some((issue) => issue.type === 'style');
...
`;
            const filePath = createTestFile(testDir, 'test.js', code);
            const result = await analyzeCode(filePath, 'javascript', {});
            const hasLogicIssue = result.codeQuality.issues.some((issue) => issue.type === 'logic');
...
`;
            const filePath = createTestFile(testDir, 'test.js', code);
            const result = await analyzeCode(filePath, 'javascript', {});
            const hasPerformanceIssue = result.codeQuality.issues.some((issue) => issue.type === 'performance');
...
`;
            ...
            const result = await analyzeCode(filePath, 'javascript', {});
            const hasSyntaxIssue = result.codeQuality.issues.some((issue) => issue.type === 'syntax');
...
`;
            ...
            const result = await analyzeCode(filePath, 'javascript', {});
            const hasTodo = result.codeQuality.issues.some((issue) => issue.message.toLowerCase().includes('todo') || issue.message.toLowerCase().includes('fixme'));
